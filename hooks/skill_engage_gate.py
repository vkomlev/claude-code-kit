#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""PreToolUse-гейт: блокирует правку исходника мимо профильного скилла.

Механический контроль поверх правила «профильный скилл авто-выбирается и выполняется ДО
работы с кодом/контентом» (см. CLAUDE.md, § «Автоподбор skills»). Правило, которое держится
только на дисциплине модели, обходится почти всегда, если скилл не назван явно. Этот хук
делает его исполняемым: правка файла проекта без задействованного скилла -> блок с подсказкой.

DATA-DRIVEN: правила НЕ захардкожены. Что считать исходником проекта и какие скиллы
профильные — в `skill_routing.json`. Новый проект = строка в JSON, правка кода не нужна.

Контракт Claude Code:
- Вход: JSON на stdin (поля cwd, hook_event_name, prompt, session_id, transcript_path,
  + tool_name, tool_input для PreToolUse).
- `transcript_path` присылается всегда — на нём держится «силансер» (проверка, что скилл
  уже задействован в этой сессии).
...
Логика:
  ...
  5. В транскрипте сессии был вызов скилла нужного домена -> пропустить.
  6. Иначе -> БЛОК с подсказкой ИМЕННО для этого проекта.

Блок снимает ЛЮБОЙ скилл своего домена (правило про ОСОЗНАННОЕ задействование, а не про
угадывание точного имени). Профильный скилл проекта называется в тексте блока (поле hint).

Fail-open: при ошибке разбора входа/реестра хук пропускает. Но «нет сигнала о скилле»
трактуется как БЛОК — иначе гейт молча выродится в не-enforcement. Обход дешёвый
(один /skill или маркер).
"""
...
def _marker_notice(path: str) -> str | None:
    ...
    mtime, и старая заметка про ЧУЖУЮ задачу может молча авторизовать сегодняшнюю
    работу в новом дереве.
    ...
    не пытается: жёсткая привязка к конкретной задаче давала бы ложные блокировки.
    """
...
#
# Если словарь захардкодить и для незнакомого домена подставлять ИНЖЕНЕРНЫЙ вариант, для
# домена infra получился бы текст «любой инженерный скилл снимает блок, напр. /executor-lite»
# — прямая ложь (executor-lite infra не открывает), толкающая агента к обходу мимо
# /claude-booster. Поэтому fallback БЕЗОПАСНЫЙ: ссылается на скиллы самого правила и ничего
# не выдумывает.
...
def evaluate(tool_name: str, path: str, transcript_path: str,
    ...
    if _override_marker_fresh(path, domain):
        return None
    # Силансит только скилл СВОЕГО домена: инженерный скилл не снимает блок с
    # маркетингового артефакта, и наоборот.
