...
Зачем проактив: корень обхода авто-подбора не только в том, что агента не останавливают,
но и в том, что он не держит в голове карту «проект -> профильный скилл». Гейт лечит
симптом, подсказка — причину.

Почему не SessionStart: сессии бывают кросс-проектные (cwd = ~/.claude, а работа в другом
каталоге), поэтому привязка к cwd ненадёжна. Триггер — упоминание проекта в промпте
...
# Компактная карта для проектов с несколькими правилами на один корень (напр. маркетинг —
# по правилу на канал). Без группировки упоминание такого проекта выдало бы много блоков
# подряд — подсказка превратилась бы в шум и её перестали бы читать.
GROUP_BLOCK = """\
