...
- После ЛЮБОЙ ротации секрета (пароль БД, API-ключ, токен) — раскатка считается завершённой только после обязательного `grep -rn "<old-marker>"` по ВСЕМ директориям ваших проектов, включая untracked/gitignored файлы — не по субъективному списку «известных потребителей» из CHANGELOG/git log.
