...
[ -f .design-artifacts/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.design-artifacts/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.design-artifacts/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.design-artifacts/no-test-bootstrap` and re-run." Continue without tests.
...
Screenshots captured via `computer{action:"screenshot"}` render inline in the response — they don't write to disk by themselves. `$REPORT_DIR/screenshots` stays as the nominal location referenced in the written report; treat inline images shown during the session as the actual before/after evidence.
...
```
.design-artifacts/design-reports/
├── design-audit-{domain}-{YYYY-MM-DD}.md    # Structured report
├── screenshots/                              # nominal paths — actual evidence is the
│                                              # inline screenshots shown during the session
...
1. `mcp__Claude_Browser__navigate` to the affected URL
2. `mcp__Claude_Browser__computer` `{action: "screenshot"}` — the "after" evidence, paired against the "before" screenshot taken during Phase 3/8a
3. `mcp__Claude_Browser__read_console_messages` `{onlyErrors: true}` — confirm no new console errors
4. `mcp__Claude_Browser__read_page` — compare structure against the pre-fix snapshot to confirm nothing else shifted
