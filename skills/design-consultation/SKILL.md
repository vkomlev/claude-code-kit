...
version: 1.1.0
description: |
  Дизайн-консультация: изучает продукт, исследует конкурентов, предлагает полную
  дизайн-систему (эстетика, типографика, цвет, раскладка, отступы, анимация) и
  генерирует страницы-превью шрифтов и цветов. Создаёт DESIGN.md как источник истины
  по дизайну проекта. Для существующих сайтов используйте /plan-design-review.
allowed-tools:
  - Bash
  - Read
  - Write
...
**Browser tooling (optional — enables visual competitive research):**

This environment provides two MCP browser tool sets, always available: `Claude_Browser` (public competitor sites, no login) and `claude-in-chrome` (sites requiring the operator's own login session — load via `ToolSearch` first, see Phase 2). Visual research stays optional: if a competitor site blocks headless access (bot detection, CAPTCHA), skip it and fall back to WebSearch and built-in design knowledge.
...
**Step 2: Visual research via Claude_Browser MCP**

Visit the top 3-5 competitor sites and capture visual evidence with `mcp__Claude_Browser__*` tools:

1. `mcp__Claude_Browser__preview_start` with `{url: "https://competitor-site.com"}` (or `navigate` if a tab is already open)
2. `mcp__Claude_Browser__computer` with `{action: "screenshot"}` — returns the image directly in the response, no separate Read step needed
3. `mcp__Claude_Browser__read_page` — structural accessibility-tree data

For each competitor, analyze: fonts actually used, color palette, layout approach, spacing density, aesthetic direction. The screenshot gives you the feel; `read_page` gives you structural data.

If a competitor site blocks the headless browser (bot detection, CAPTCHA) or requires login, skip it and note why — this is expected for some public sites and the research still works with fewer samples.

If Claude_Browser research is skipped or blocked across the board, rely on WebSearch results and your built-in design knowledge — this is fine.
...
- Claude_Browser reachable → screenshots + `read_page` + WebSearch (richest research)
- Site blocks headless access → WebSearch only for that competitor (still good)
