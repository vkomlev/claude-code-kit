...
2. `~/.claude/ERRORS.md` — глобальный реестр (если проектного нет).
...
5. **Покрытие тестами** — адекватность, граничные случаи; **mock-only на external write-path = блокирующий FAIL** (без live smoke с env-gate) — см. [api-contract-rules.md §7-8](../claude-booster/references/api-contract-rules.md). Если изменение **user-facing и уже задеплоено** — проверить его **на живой странице самому**. Зелёные тесты + верный код ≠ работает у пользователя: типичный разрыв — бэкенд не засчитывает действие без вложения, а UI отправку не блокирует. **ПРИНЯТО по живому поведению без живой проверки — это гипотеза, а не вердикт.**
6. **Docs/Config/Runtime Drift** — документация соответствует коду? README, CLAUDE.md, конфиги актуальны? При переименовании ADR (rename/renumber) — `grep -rn "ADR-<старый_номер>\|<старый_номер>-<slug>"` по `docs/ tests/ scripts/` обязан вернуть **0 совпадений** (иначе читатель попадает не в тот ADR) — см. реестр ошибок проекта.
7. **Phase Integrity** — все фазы работы завершены? Нет TODO, заглушек, незакрытых задач? **Scope staged-набора ⊆ scope активной спеки**: `git diff --cached --stat` сверить со списком файлов review-артефакта; смешение нескольких фаз/fix в одном staged-наборе без явного `bundled`-разрешения spec → **АВТОМАТИЧЕСКИ ОТКЛОНЕНО** (источник: реестр ошибок проекта).
...
11. **Cross-project memory sync** — если изменение затрагивает несколько модулей и вы ведёте документацию состояния смежных модулей (контракты, changelog, состояние): обновлены ли они? Без update — **АВТОМАТИЧЕСКИ ОТКЛОНЕНО**.
...
- Cross-project зависимости между вашими модулями: любое изменение опубликованных контрактов
...
```
Классы: PROCESS, DATA, LOGIC, INTEGRATION.
