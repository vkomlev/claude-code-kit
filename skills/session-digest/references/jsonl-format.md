...
- `~/projects/content-project` → `D--Work-content-project` (диск в upper case, `\` и `:` → `-`)
- `~/projects/course-project` → `D--Work-IT-Businessman` (подчёркивание → дефис)
- `~\.openclaw` → `C--Users-user--openclaw` (точка → дефис, двойной дефис для dot-folder)
...
- `timestamp` — ISO 8601 в UTC. Для дневной суммаризации фильтровать по дате локальной зоны владельца (МСК по умолчанию для владельца).
...
- **Side-chain сессии:** `isSidechain:true` означает суб-агента (Task tool). Для дайджеста владельца обычно интересен основной поток — фильтровать по `isSidechain:false` или учитывать отдельно
